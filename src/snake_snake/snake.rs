...
/// 蛇身体的颜色
const SNAKE_BODY_COLOR: Color = [0.5, 0.8, 0.5, 1.0];
/// 蛇头的颜色
const SNAKE_HEAD_COLOR: Color = [0.0, 1.0, 0.0, 1.0];
...
impl Snake {
    ...
    pub fn draw(&self, con: &Context, g: &mut G2d) {
        ...
        for block in &self.body {
            if is_head {
                is_head = false;
                draw_block(SNAKE_HEAD_COLOR, Shape::Square, block.x, block.y, con, g);
            } else {
                draw_block(SNAKE_BODY_COLOR, Shape::Square, block.x, block.y, con, g);
            }
        }
    }
